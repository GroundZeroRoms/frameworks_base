...
public class CustomTile extends QuickSettingsTile {
    ...
    public CustomTile(Context context, QuickSettingsController qsc, String key) {
        ...
        qsc.registerObservedContent(Settings.System.getUriFor(
                Settings.System.CUSTOM_TOGGLE_MATCH_STATE), this);
    }
    ...
    private void updateSettings() {
        ...
        String matchState = getActionsFromString(
                Settings.System.getStringForUser(mContext.getContentResolver(),
                Settings.System.CUSTOM_TOGGLE_MATCH_STATE, UserHandle.USER_CURRENT));
        if (matchState != null && matchState.equals("true")) {
            mMatchState = true;
        } else {
            mMatchState = false;
        }
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (clickHolder != null || longHolder != null) {
    ...
    private void performAction(boolean longPress) {
        if (!longPress) {
            ...
            if (mMatchState) {
                SlimActions.processAction(mContext, mClickActions[mStateMatched], false);
            } else {
                SlimActions.processAction(mContext, mClickActions[mState], false);
            }
            updateResources();
        } else {
            SlimActions.processAction(mContext, mLongActions[mState], false);
        }
    }
